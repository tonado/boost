...
{
    typename C::size_type old_size = std::distance(c.begin(), c.end());
    ...
    if (p.second)
    {
        ...
        BOOST_CHECK(std::distance(c.begin(), c.end()) == old_size + 1);
    }
    else
    {
        BOOST_CHECK(std::distance(c.begin(), c.end()) == old_size);
    }
}
