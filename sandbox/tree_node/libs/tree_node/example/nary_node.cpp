...
template <typename DItr, typename AItr>
void out_the_splice(DItr d_itr, DItr d_end, AItr a_itr)
{
    std::cout << "        a_src_itr = boost::begin(a_splice_result);";
    std::cout << std::endl << (
    ...
    for (; d_itr != d_end; ++d_itr)
    {
        ...
        out_with_them(*d_itr, *a_itr);
        std::cout << "        test_node<a_node_case>(*a_src_itr, bf_vals, ";
        std::cout << "pre_vals, post_vals, df_vals);" << std::endl;
        std::cout << "        ++a_src_itr;" << std::endl;
        ++a_itr;
    }

    std::cout << "        BOOST_CHECK(a_src_itr == a_end_itr);" << std::endl;
}
...
template <typename DItr>
void out_the_splice(DItr d_itr, DItr d_end)
{
    std::cout << "        a_src_itr = boost::begin(a_splice_result);";
    std::cout << std::endl << (
        "        a_end_itr = boost::end(a_splice_result);"
...
void example()
{
    ...
    {
        DNode d_copy(d_root);
        DNode d_move(boost::move(d_copy));
        ...
        std::cout << std::endl << (
            "#if !defined BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION"
        ) << std::endl << "        ANode a_move(boost::move(a_copy));";
        std::cout << std::endl << (
            "        boost::tree_node::breadth_first_iterator<ANode const>"
        ) << std::endl << (
        ...
        std::cout << "_descendant_iterator<ANode const>" << std::endl << (
            ...
            "test_node_copies(bf_d_same_itr, bf_d_move_itr));"
        ) << std::endl << "        BOOST_CHECK(" << (
            "test_node_copies(pre_same_itr, pre_move_itr));"
        ) << std::endl << "        BOOST_CHECK(" << (
            "test_node_copies(pre_d_same_itr, pre_d_move_itr));"
        ) << std::endl << "        BOOST_CHECK(" << (
            "test_node_copies(post_same_itr, post_move_itr));"
        ) << std::endl << "        BOOST_CHECK(" << (
            "test_node_copies(post_d_same_itr, post_d_move_itr));"
        ) << std::endl << (
            "        BOOST_CHECK(test_node_copies(df_same_itr, df_move_itr));"
        ) << std::endl << "        BOOST_CHECK(" << (
            "test_node_copies(df_d_same_itr, df_d_move_itr));"
        ) << std::endl << "        BOOST_CHECK(a_copy.empty());" << std::endl;
        std::cout << std::endl << (
            "        Values bf_vals, pre_vals, post_vals;"
        ) << std::endl << "        DFValues df_vals;" << std::endl;
        std::cout << "        Emplacer emplacer;" << std::endl;
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        out_with_them(d_copy, a_copy);
#else
        out_with_it(d_copy);
#endif
        std::cout << "        test_node<a_node_case>(a_copy, bf_vals, pre_";
        ...
        std::cout << "        Emplacer emplacer;" << std::endl;
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        output_full_node_test(*a_child_itr, "*a_child_itr");
        out_with_them(d_root, a_root);
#else
        output_node_test(*d_child_itr, "*a_child_itr");
        out_with_it(d_root);
#endif
        ...
        *a_child_itr = a_root;
        ...
        typename DNode::iterator d_desc_itr = (*d_child_itr).begin();

        std::advance(d_desc_itr, 4);

        typename DNode::iterator d_src_itr = (*d_desc_itr).begin();
        typename DNode::iterator d_end_itr = ++d_src_itr;
        typename boost::insert_range_result_gen<
            Selector
          , DNode
        >::type d_splice_result = (*d_child_itr).splice(
            ++(*d_child_itr).begin()
          , *d_desc_itr
          , d_src_itr
          , ++++d_end_itr
...
#endif
        std::cout << "        typename ANode::iterator a_desc_itr = ";
        std::cout << "(*a_child_itr).begin();" << std::endl << std::endl;
        std::cout << "        std::advance(a_desc_itr, 4);" << std::endl;
        std::cout << std::endl << "        typename ANode::iterator a_src_itr";
        std::cout << " = (*a_desc_itr).begin();" << std::endl << (
            "        typename ANode::iterator a_end_itr = ++a_src_itr;"
        ) << std::endl << "        typename boost::insert_range_result_gen<";
        std::cout << std::endl << "            Selector" << std::endl;
        std::cout << "          , ANode" << std::endl << (
            "        >::type a_splice_result = (*a_child_itr).splice("
        ) << std::endl << "            ++(*a_child_itr).begin()" << std::endl;
        std::cout << "          , *a_desc_itr" << std::endl;
        std::cout << "          , a_src_itr" << std::endl;
        std::cout << "          , ++++a_end_itr" << std::endl;
        std::cout << "        );" << std::endl;
        out_the_splice(
            boost::begin(d_splice_result)
          , boost::end(d_splice_result)
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
          , boost::begin(a_splice_result)
#endif
        );
        std::cout << "        bf_vals.clear();" << std::endl;
        std::cout << "        pre_vals.clear();" << std::endl;
        ...
        d_desc_itr = (*d_child_itr).begin();
        d_src_itr = ++++d_desc_itr;
        d_end_itr = ++d_src_itr;
        d_splice_result = (*d_child_itr).splice(
            d_desc_itr
          , *d_child_itr
          , d_src_itr
          , ++++++d_end_itr
...
#endif
        std::cout << "        a_desc_itr = (*a_child_itr).begin();";
        std::cout << std::endl << "        a_src_itr = ++++a_desc_itr;";
        std::cout << std::endl << "        a_end_itr = ++a_src_itr;";
        std::cout << std::endl << (
            "        a_splice_result = (*a_child_itr).splice("
        ) << std::endl << "            a_desc_itr" << std::endl;
        ...
        d_src_itr = (*d_child_itr).begin();
        d_end_itr = ++++d_src_itr;
        d_desc_itr = ++++++d_end_itr;
        d_splice_result = (*d_child_itr).splice(
            ++d_desc_itr
          , *d_child_itr
          , d_src_itr
...
#endif
        std::cout << "        a_src_itr = (*a_child_itr).begin();";
        std::cout << std::endl << "        a_end_itr = ++++a_src_itr;";
        std::cout << std::endl << "        a_desc_itr = ++++++a_end_itr;";
        std::cout << std::endl << (
            "        a_splice_result = (*a_child_itr).splice("
        ) << std::endl << "            ++a_desc_itr" << std::endl;
        std::cout << "          , *a_child_itr" << std::endl;
    ...
    std::cout << "        test_node<f_node_case>(f_root, bf_vals, pre_vals,";
    std::cout << " post_vals, df_vals);" << std::endl << "    }" << std::endl;
    std::cout << "#endif  // defined BOOST_TREE_NODE_CAN_USE_FUSION";
    std::cout << std::endl;
#endif  // defined BOOST_TREE_NODE_CAN_USE_FUSION
    ...
    std::cout << std::endl << (
        "#endif  // !defined BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION"
    ) << std::endl << "#endif  // defined BOOST_TYPEOF_EMULATION" << std::endl;
...
#if defined BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
#include <boost/typeof/boost/ptr_container/ptr_vector.hpp>
#elif defined LIBS_TREE_NODE_EXAMPLE_NARY_NODE_USE_DEQUE
