...
int main()
{
    ...
    std::cout << std::endl;
    std::cout << "#include <boost/tree_node/algorithm/binary_descendant.hpp>";
...

    {
        ...
        std::cout << "_descendant_iterator<ANode const>" << std::endl << (
            ...
            "test_node_copies(in_same_itr, in_move_itr));"
        ) << std::endl << (
...
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        output_full_node_test(*a_child_itr, "*p_child_itr");
        out_with_them(d_root, a_root);
#else
        output_node_test(*d_child_itr, "*p_child_itr");
        out_with_it(d_root);
#endif
        std::cout << "        test_node<a_node_case>(" << std::endl;
        ...
        a_root = *a_child_itr;
#endif
        std::cout << "        a_root = *p_child_itr;" << std::endl;
    ...
    {
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        ANode::pointer a_p(a_root.get_right_child_ptr()->rotate_left());

        d_root.get_right_child_ptr()->rotate_left();
#else
        DNode::pointer d_p(d_root.get_right_child_ptr()->rotate_left());
#endif
        std::cout << "    {" << std::endl << "        ANode::pointer p(a_root";
        std::cout << ".get_right_child_ptr()->rotate_left());" << std::endl;
        std::cout << "        Values bf_vals, pre_vals, post_vals, in_vals;";
        std::cout << std::endl << "        DFValues df_vals;" << std::endl;
        std::cout << "        Emplacer emplacer;" << std::endl;
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        output_full_node_test(*a_p, "*p");
        out_with_them(d_root, a_root);
#else
        output_node_test(*d_p, "*p");
        out_with_it(d_root);
#endif
        std::cout << "        test_node<a_node_case>(" << std::endl;
        std::cout << "            a_root" << std::endl;
    ...
    {
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        ANode::pointer a_p(a_root.get_left_child_ptr()->rotate_right());

        d_root.get_left_child_ptr()->rotate_right();
#else
        DNode::pointer d_p(d_root.get_left_child_ptr()->rotate_right());
#endif
        std::cout << "    {" << std::endl << "        ANode::pointer p(a_root";
        std::cout << ".get_left_child_ptr()->rotate_right());" << std::endl;
        std::cout << "        Values bf_vals, pre_vals, post_vals, in_vals;";
        std::cout << std::endl << "        DFValues df_vals;" << std::endl;
        std::cout << "        Emplacer emplacer;" << std::endl;
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
    ...
    {
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        ANode::pointer a_p(a_root.get_left_child_ptr()->rotate_left());

        d_root.get_left_child_ptr()->rotate_left();
#else
        DNode::pointer d_p(d_root.get_left_child_ptr()->rotate_left());
#endif
        std::cout << "    {" << std::endl << "        ANode::pointer p(a_root";
        std::cout << ".get_left_child_ptr()->rotate_left());" << std::endl;
        std::cout << "        Values bf_vals, pre_vals, post_vals, in_vals;";
        std::cout << std::endl << "        DFValues df_vals;" << std::endl;
        std::cout << "        Emplacer emplacer;" << std::endl;
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
...
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        ANode::pointer a_p(
            a_root.get_right_child_ptr()->emplace_parent_of_left(8)
        );

        d_root.get_right_child_ptr()->emplace_parent_of_left(8);
#else
        DNode::pointer d_p(
            d_root.get_right_child_ptr()->emplace_parent_of_left(8)
        );
...
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        ANode::pointer a_p(
            a_root.get_left_child_ptr()->emplace_parent_of_right(9)
        );

        d_root.get_left_child_ptr()->emplace_parent_of_right(9);
#else
        DNode::pointer d_p(
            d_root.get_left_child_ptr()->emplace_parent_of_right(9)
        );
...
#if !defined BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
    *d_root.get_left_child_ptr() = boost::move(*d_root.get_right_child_ptr());
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
    *a_root.get_left_child_ptr() = boost::move(*a_root.get_right_child_ptr());
#endif
    std::cout << std::endl << (
        "#if !defined BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION"
    ) << std::endl << "    *a_root.get_left_child_ptr() = ";
    std::cout << "boost::move(*a_root.get_right_child_ptr());" << std::endl;
    std::cout << std::endl << "    {" << std::endl;
    std::cout << "        Values bf_vals, pre_vals, post_vals, in_vals;";
    std::cout << std::endl << "        DFValues df_vals;" << std::endl;
    std::cout << "        Emplacer emplacer;" << std::endl;
    ...
    std::cout << std::endl << "          , df_vals" << std::endl;
    std::cout << "        );" << std::endl << "    }" << std::endl;
    std::cout << std::endl << "    //[example__gui__binary_node__type";
    std::cout << std::endl << (
        "    typedef boost::tree_node::with_accumulation<"
    ) << std::endl << (
        "                boost::tree_node::with_accumulation_base_gen<"
    ...
    std::cout << std::endl << "    for (" << std::endl << "        boost" << (
        "::tree_node::breadth_first_descendant_iterator<GUINode const> itr("
    ) << std::endl << "            gui_root" << std::endl << "        );";
    std::cout << std::endl << "        itr;" << std::endl << "        ++itr";
    std::cout << std::endl << "    )" << std::endl << "    {" << std::endl;
    std::cout << "        BOOST_CHECK(" << std::endl;
    std::cout << "            get(*itr->get_parent_ptr(), MinPriorityKey())";
    std::cout << std::endl << "         <= get(*itr, MinPriorityKey())";
    std::cout << std::endl << "        );" << std::endl << "    }";
    std::cout << std::endl << std::endl;

    typename GUINode::pointer gui_ptr;

    for (unsigned int number = 0; number < 112; ++number)
    {
        gui_ptr = binary_descendant(&gui_root, number, GUIBinaryCompare());

        if (gui_ptr)
        {
            std::cout << "    BOOST_CHECK(" << std::endl << "        ";
            std::cout << get(*gui_ptr, example_keys::row_number());
            std::cout << " == get(" << std::endl;
            std::cout << "            *binary_descendant(&gui_root, ";
            std::cout << number << ", GUIBinaryCompare())" << std::endl << (
                "          , example_keys::row_number()"
            ) << std::endl << "        )" << std::endl;
            std::cout << "    );" << std::endl;
        }
