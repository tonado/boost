...
void output_driver()
{
...

    {
        ...
        std::cout << "_descendant_iterator<ANode const>" << std::endl << (
            ...
            "test_node_copies(bf_same_itr, bf_move_itr));"
        ) << std::endl << "        BOOST_CHECK(" << std::endl << "      " << (
            "      test_associative_node_copies(bf_d_same_itr, bf_d_move_itr)"
        ) << std::endl << "        );" << std::endl << "        " << (
            "BOOST_CHECK(test_node_copies(pre_same_itr, pre_move_itr));"
        ) << std::endl << "        BOOST_CHECK(" << std::endl << "       " << (
            "     test_associative_node_copies(pre_d_same_itr, pre_d_move_itr)"
        ) << std::endl << "        );" << std::endl << "        " << (
            "BOOST_CHECK(test_node_copies(post_same_itr, post_move_itr));"
        ) << std::endl << "        BOOST_CHECK(" << std::endl << "       " << (
            "     test_associative_node_copies(post_d_same_itr, post_d_move_it"
        ) << "r)" << std::endl << "        );" << std::endl << "        " << (
            "BOOST_CHECK(test_node_copies(df_same_itr, df_move_itr));"
        ) << std::endl << "        BOOST_CHECK(" << std::endl << "       " << (
            "     test_associative_node_copies(df_d_same_itr, df_d_move_itr)"
        ) << std::endl << "        );" << std::endl;
        std::cout << "        BOOST_CHECK(a_copy.empty());" << std::endl;
        std::cout << std::endl << (
        ...
        std::cout << std::endl << "            a_copy" << std::endl;
        std::cout << "          , bf_vals" << std::endl;
        std::cout << "          , bf_d_vals" << std::endl;
        std::cout << "          , pre_vals" << std::endl;
        std::cout << "          , pre_d_vals" << std::endl;
        ...
        std::cout << "        BOOST_CHECK(names[5] == a_child_itr->first);";
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        output_full_node_test_long(
            boost::tree_node::dereference_iterator(a_child_itr)
          , "boost::tree_node::dereference_iterator(a_child_itr)"
        );
        out_with_them(d_root, a_root);
#else
        output_node_test_long(
            boost::tree_node::dereference_iterator(d_child_itr)
          , "boost::tree_node::dereference_iterator(a_child_itr)"
        );
        out_with_it(d_root);
#endif
...
int main()
{
    ...
    std::cout << std::endl << '{' << std::endl << (
        ...
        "        boost::ptr_map_selector<boost::c_str_less_than_selector>"
    ) << std::endl << "    >();" << std::endl << (
        "    test_multiple_associative<"
    ) << std::endl << (
        "        boost::ptr_multimap_selector<boost::c_str_less_than_selector>"
    ) << std::endl << "    >();" << std::endl;
    ...
    std::cout << "boost::c_str_less_than_selector>" << std::endl << "    >();";
    ...
    std::cout << "    test_unique_associative<" << std::endl << (
        "        boost::flat_map_selector<boost::c_str_less_than_selector>"
    ) << std::endl << "    >();" << std::endl << (
        "    test_multiple_associative<"
    ) << std::endl << "        boost::flat_multimap_selector<" << std::endl;
    std::cout << "boost::c_str_less_than_selector>" << std::endl << "    >();";
    std::cout << std::endl << "#endif" << std::endl;
