...
//      , "Parent member uninitialized."
...

    {
        ...
        BOOST_CHECK(
            test_associative_node_copies(post_d_same_itr, post_d_move_itr)
        );
...

    {
        ...
        BOOST_CHECK(names[5] == a_child_itr->first);
        BOOST_CHECK(
            5 == get(
...
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        BOOST_CHECK(
            5 == boost::tree_node::get<boost::tree_node::accumulation_key<> >(
                boost::tree_node::dereference_iterator(a_child_itr)
            ).numerator()
        );
        BOOST_CHECK(
...
#if defined BOOST_TREE_NODE_CAN_USE_FUSION
        BOOST_CHECK(
            55 == boost::tree_node::get<boost::tree_node::accumulation_key<> >(
                boost::tree_node::dereference_iterator(a_child_itr)
            ).numerator()
        );
        BOOST_CHECK(
...
#if !defined BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
    test_unique_associative<
        boost::map_selector<boost::mpl::true_,boost::c_str_less_than_selector>
    >();
    test_multiple_associative<
