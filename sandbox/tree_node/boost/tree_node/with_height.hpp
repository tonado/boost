...
#undef BOOST_TREE_NODE_EMPLACEMENT_IMPL
    ...
    {
        super_t::on_post_erase_impl(does_not_invalidate_child_positions);
        ...
        >::_update_less_height(pointer p)
    {
        for (
            ...
            (p = p->get_parent_ptr());
            p->_height = new_height  // This is no longer the deepest branch.
        )
        {
        ...
        >::_update_greater_height(pointer p)
    {
        for (
            ...
            (p = p->get_parent_ptr()) && (p->_height < ++p_height);
            p->_height = p_height  // This is the new deepest branch.
        )
        {
        }
    }
